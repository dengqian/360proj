...
class stat_struct:
    ...
    def update_state(self, pd):
        if pd.dir == DIR_IN and pd.leng > 0:
            self.data_num += 1
            self.pre_flag = 'D'
        elif self.pre_flag == 'D':
            self.pkt_num.append(self.data_num)
            self.data_num = 0
...
def parse_vse_conn(pd, flag):
    ...
    if stat.tcp_state == TCP_CLOSED:
        #print pd.key
        if flag == 'wifi' and wifi.count(pd.key) > 0:
            print pd.key+'\t'+str(stat.pkt_num)
        if flag == 'td' and td.count(pd.key) > 0:
            print pd.key+'\t'+str(stat.pkt_num)
        if flag == 'edge' and edge.count(pd.key) > 0:
            print pd.key+'\t'+str(stat.pkt_num)
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        get_ip_list('edge_v3.txt', 'edge')
        get_ip_list('td_v3.txt', 'td')
        get_ip_list('wifi_v3.txt', 'wifi')
